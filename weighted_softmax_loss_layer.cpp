...
void WeightedSoftmaxWithLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
      ...
      // compute loss
      loss -= weights_[label_value] * log(std::max(prob_data[i * dim + label_value * inner_num_ + j],
                           Dtype(FLT_MIN)));
      ++count;
    }
  }
      ...
      for (int j = 0; j < inner_num_; ++j) {
        ...
        if (has_ignore_label_ && label_value == ignore_label_) {
          for (int c = 0; c < bottom[0]->shape(softmax_axis_); ++c) {
            bottom_diff[i * dim + c * inner_num_ + j] = 0; // ignore label, diff == 0
          }
        } else {
          // compute diff
          bottom_diff[i * dim + label_value * inner_num_ + j] -= 1;
          for (int c = 0; c < bottom[0]->shape(softmax_axis_); ++c) {
            bottom_diff[i * dim + c * inner_num_ + j] *= weights_[label_value];
          }
          ++count;
        }
      }
