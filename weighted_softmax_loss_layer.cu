...
__global__ void WeightedSsoftmaxLossForwardGPU(const int nthreads,
          const Dtype* prob_data, const Dtype* label, Dtype* loss,
          const int num, const int dim, const int spatial_dim,
          const bool has_ignore_label_, const int ignore_label_,
          Dtype* counts, const Dtype* weights) {
...
void WeightedSoftmaxWithLossLayer<Dtype>::Forward_gpu(
  ...
  // cudaMalloc((void**)&weights, bottom[0]->channels() * sizeof(Dtype));
  // cudaMemcpy(weights, weights_, bottom[0]->channels()* sizeof(Dtype), cudaMemcpyHostToDevice);
  // cudaFree(weights);
  WeightedSsoftmaxLossForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
      CAFFE_CUDA_NUM_THREADS>>>(nthreads, prob_data, label, loss_data,
      outer_num_, dim, inner_num_, has_ignore_label_, ignore_label_, counts, gpu_weights_);
  Dtype loss;
...
__global__ void WeightedSoftmaxLossBackwardGPU(const int nthreads, const Dtype* top,
          ...
          const int ignore_label_, Dtype* counts, const Dtype* weights) {
